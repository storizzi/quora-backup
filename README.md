...
- If you are using the answers.json file for other purposes, then please note that the datePosted attribute is only approximate, based on the rough time frame that appears with each post (e.g. '5 weeks ago' or '3 months ago' or '1 year ago').
- If you want to now get markdown versions of the html items you have previously crawled, then when you set the environment variable to enabled markdown creation and run the script, it will work off the previously created html files if it can without re-crawling.
